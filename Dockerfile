...
FROM --platform=$BUILDPLATFORM demonstrationorg/dhi-node:24-debian13-sfw-dev_backstage AS packages
WORKDIR /app
...
FROM --platform=$BUILDPLATFORM demonstrationorg/dhi-node:24-debian13-sfw-dev_backstage AS build-packages
...
FROM --platform=$BUILDPLATFORM demonstrationorg/dhi-node:24-debian13-sfw-dev_backstage AS build-app
ENV PYTHON=/opt/python/bin/python3
WORKDIR /app
...
FROM demonstrationorg/dhi-node:24-debian13_backstage
ENV PYTHON=/opt/python/bin/python3
WORKDIR /app
